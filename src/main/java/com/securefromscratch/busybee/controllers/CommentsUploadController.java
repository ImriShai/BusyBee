...
public class CommentsUploadController {
    ...
    public ResponseEntity<CreatedCommentId> addComment(
    ...
    ) throws IOException, TypeValidationException, SecurityException, TaskNotFoundException, BadRequestException, NotEnoughSpaceException {
        ...
        String userId = "Yariv"; // Replace with actual user ID if user accounts are implemented
        ...
        if (optFile.isPresent() && !optFile.get().isEmpty()) {
            FileStorage.StoredFile storedFile = fileStorage.store(optFile.get(), userId);
